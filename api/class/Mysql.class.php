...
class Mysql {
    ...
    public function cacheObj($cache) {
        $this->cache	= $cache;
    }
...
//         $this->close();
    ...
    //从缓存中获取数据
    protected function getCache($sql,$method) {
    ...
    //缓存单个字段
    public function cacheOne($sql, $reload = false) {
        $this->reload	= $reload;
    ...
    //缓存行
    public function cacheRow($sql, $reload = false) {
        $this->reload	= $reload;
        $sql    = $this->get_query_sql($sql, 1);
    ...
    //缓存all
    public function cacheAll($sql, $reload = false, $limit = null) {
        $this->reload	= $reload;
        $sql    = $this->get_query_sql($sql, $limit);
     ...
     * @param    array      $info       数据
     */
    public function getReplaceDbSql($tbl_name,$info)
    {
        if(is_array($info)&&!empty($info))
        {
            $i = 0;
     ...
     */
    public function getUpdateDbSql($tbl_name,$info,$condition)
    {
        $i = 0;
        $data = '';
        if(is_array($info)&&!empty($info))
    ...
    public function realGet($sql, $fetch_mode = MYSQLI_ASSOC) {
        $this->query($sql);
        $this->fetch_mode    = $fetch_mode;
        $row = $this->fetch();
        $this->free();
